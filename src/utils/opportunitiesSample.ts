...
export function sampleSetFlags(id: number, flags: { flaggedByUser?: string; rejectedByUser?: string }): { opportunityId: number; flaggedByUser: string; rejectedByUser: string } | null {
  const row = SAMPLE_ROWS.find((r) => r.opportunityId === id);
  if (!row) return null;
  if (flags.flaggedByUser) row.flaggedByUser = flags.flaggedByUser;
  if (flags.rejectedByUser) row.rejectedByUser = flags.rejectedByUser;
  return { opportunityId: id, flaggedByUser: String(row.flaggedByUser ?? "N"), rejectedByUser: String(row.rejectedByUser ?? "N") };
}

export type SampleDocumentT = {
  objectId: string;
...
const SAMPLE_DOCS: SampleDocumentT[] = [
  { objectId: "obj-1001-a", opportunityId: 1001, displayName: "W91ZLK-26-R-0001.pdf", documentType: "solicitation", size: 1834221, uploadedBy: "AsterionDB Administrator", uploadedAt: "2026-09-16T16:46:48" },
  { objectId: "obj-1001-b", opportunityId: 1001, displayName: "Amendment-001.pdf", documentType: "amendment", size: 241120, uploadedBy: "AsterionDB Administrator", uploadedAt: "2026-09-17T09:12:00" },
  { objectId: "obj-1008-a", opportunityId: 1008, displayName: "HC1028-26-R-0031.pdf", documentType: "solicitation", size: 945000, uploadedBy: "AsterionDB Administrator", uploadedAt: "2026-09-15T14:30:00" },
  { objectId: "obj-lib-1", opportunityId: null, displayName: "Past-Performance-Summary.pdf", documentType: "past-performance", size: 512000, uploadedBy: "AsterionDB Administrator", uploadedAt: "2026-09-10T10:00:00" },
];

export function sampleGetDocuments(opportunityId: number | null): SampleDocumentT[] {
  if (opportunityId === null) return [...SAMPLE_DOCS].sort((a, b) => b.uploadedAt.localeCompare(a.uploadedAt));
  return SAMPLE_DOCS.filter((d) => d.opportunityId === opportunityId).sort((a, b) => b.uploadedAt.localeCompare(a.uploadedAt));
}

export function sampleAttachDocument(doc: Omit<SampleDocumentT, "uploadedBy" | "uploadedAt">): SampleDocumentT {
  const record: SampleDocumentT = {
    ...doc,
    uploadedBy: "AsterionDB Administrator",
    uploadedAt: new Date().toISOString().slice(0, 19),
  };
  SAMPLE_DOCS.unshift(record);
  return record;
}
